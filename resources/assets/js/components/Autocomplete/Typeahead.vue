...
<template>
    ...
    <div class="form-group" :class="{ 'has-feedback': loading }">
        <label>{{ label }}</label>
        <input class="form-control" type="search" autocomplete="off" v-model="query"
              ...
              @focus="focus"
          />
          <i v-show="loading" class="glyphicon glyphicon-refresh form-control-feedback"></i>
      </div>

      <ul class="dropdown-menu left-margin" v-show="show && hasItems">
...
export default {
  ...
  methods: {
    ...
    hit: function (e) {
      ...
      if (this.current < 0){
          this.$dispatch('on-select-with-no-results');
        this.reset();
        return;
      }
